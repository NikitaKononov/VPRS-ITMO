...
class PlateFinder:
    def __init__(self, minConfidence, categoryIdx, charIOUMax=0.3, charPlateIOAMin=0.5, rejectPlates=False,
                 minScore=0.6, minChars=2):
    ...
    def processPlates(self, plates, plateBoxes, plateScores):
        ...
        for (i, (plateBox, plateScore, chScores)) in enumerate(zip(plateBoxes, plateScores, charScores)):
            ...
            if averageScore < self.minScore or len(chScores) <= self.minChars or max(plateBox) >= 0.998 or min(
                    plateBox) <= 0.002:
                mask[i] = False
        ...
        if (len(plateBoxes) != len(plateAverageScores) or len(plateBoxes) != len(charTexts)):
            print(
                "[ERROR]: len(platesBoxes):{} != len(plateAverageScores):{} or len(platesBoxes):{} != len(charTexts):{}"
    ...
    def findPlates(self, boxes, scores, labels):
        ...
        plateBoxes, charTexts, charBoxes, charScores, plateAverageScores = self.processPlates(plates, plateBoxes,
                                                                                              plateScores)
        if len(plateBoxes) != 0:
            licensePlateFound = True
